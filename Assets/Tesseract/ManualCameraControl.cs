...
public class ManualCameraControl : MonoBehaviour
{
	...
	public Vector3 Position
	{
		get
		{
			...
			{
				return newParent.transform.position;
			}
	...
	private void Update()
	{
		...
		if (!lockMovement)
		{
			if (VRSettings.loadedDevice != VRDeviceType.None)
			{
				newParent.transform.Translate(translate, Space.World);
				newParent.transform.Translate(this.lastTrackerToUnityTranslation, Space.World);
			}
			else
			{
	...
	internal void SetPosition(Vector3 savedPlayerPosition)
	{
		if (VRSettings.loadedDevice != VRDeviceType.None)
		{
			newParent.transform.position = savedPlayerPosition;
		}
		else
		{
